...
class TimeSpentRepo extends Component {
    createPDF=async()=> {
    ...
    RNFS.moveFile(decodedURL, RNFS.DownloadDirectoryPath+"/TimeSpent.pdf")
...
<div style="overflow-x:auto;">
   ...
   <th style="width:100%;text-align: center; border: 1px solid #000000; colspan="23"">Time Spent by Visitors</th>
   <tr style= "background-color: #154c79">
        ...
        <td style="color:#ffffff; border: 1px solid #000000;text-align: center;padding: 8px">InTime</td>
        <td style="color:#ffffff; border: 1px solid #000000;text-align: center;padding: 8px">OutTime</td>
  ...
  getData = async () => {
    ...
    try {
      let response = await axiosAuthGet(
        ...
        var duration=null;
     var newData= await response.filter(x=>
    ...
    for (let i in this.state.data) {
      ...
      t = t +
       `
        ...
        <td>${item?.checkInTime!=null&&moment(item?.checkInTime).format("DD/MM/YYYY hh:mm:ss")}</td>
        <td>${item?.checkOutTime!=null&&moment(item?.checkOutTime).format("DD/MM/YYYY hh:mm:ss")}</td>
...
export default connect(mapStateToProps)(TimeSpentRepo);
